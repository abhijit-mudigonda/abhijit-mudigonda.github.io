...
- **Circuit Complexity**
    - Can we bound the uniform versions of \\(\mathsf{VP}\\) and \\(\mathsf{VNP}\\) with uniform families of Boolean circuits? There is a corresponding [result][cook-valiant] for the nonuniform versions, that more or less shows that the "Boolean part" of \\(\mathsf{VP}\\) - the Boolean functions computable by things in \\(\mathsf{VP}\\) - is contained between nonuniform \\(\mathsf{NC^1}\\) and \\(\mathsf{NC^2}\\) and that the Boolean part of \\(\mathsf{VNP}\\) equals \\(\mathsf{GapP}\\).  **(Yes! )**
